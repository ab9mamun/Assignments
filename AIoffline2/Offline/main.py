...
def cost_func(state):
    cost = 0
    for slot in state:
        teacher_count = [0] * (max_teacher+1)
        class_count = [0] * (max_class+1)
        room_count = [0] * (max_room+1)
        ...
        for count in teacher_count:
            cost = cost + max(count - 1, 0)*weight_vector[0]
        for count in class_count:
            cost = cost + max(count - 1, 0)*weight_vector[1]
        for count in room_count:
            cost = cost + max(count - 1, 0)*weight_vector[2]
...
def initialize():
    ...
    for line in content:
       ...
       # print(words)
        if words[0] == "Timetable":
            ...
            max__class = max(cls,max__class)
            max__teacher = max(teacher,max__teacher)
            max__room = max(room, max__room)
    ...
    return periods, days, weight__vector, require_ments, max__class, max__teacher, max__room


start_time = time.time()


periods_per_day, days_per_week, weight_vector, requirements, max_class, max_teacher, max_room = initialize()
#print(periods_per_day, days_per_week, weight_vector)
num_slots = periods_per_day*days_per_week
req_length = 3
random_restart_limit = 500
neighbor_attempt_limit = 300
